...
export default function Home() {
  return (
    <SiteShell>
        ...
        <Container className="relative flex min-h-[82vh] flex-col justify-center pb-24 pt-20 sm:min-h-[88vh] sm:pb-28">
          <div className="max-w-3xl">
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <OutlineCta href="/services" dark>Explore Services</OutlineCta>
            </div>
            ...
            <dl className="mt-12 flex flex-wrap items-center gap-x-10 gap-y-6 border-t border-background/15 pt-8">
              <div className="flex items-center gap-3"><dd className="text-2xl font-semibold tracking-tight text-background sm:text-3xl">5.0★</dd><dt className="max-w-[6.5rem] text-xs font-medium uppercase tracking-wider text-background/60">Google rating</dt></div>
              <div className="flex items-center gap-3"><dd className="text-2xl font-semibold tracking-tight text-background sm:text-3xl">15+</dd><dt className="max-w-[6.5rem] text-xs font-medium uppercase tracking-wider text-background/60">Winnipeg reviews</dt></div>
              <div className="flex items-center gap-3"><dd className="text-2xl font-semibold tracking-tight text-background sm:text-3xl">24/7</dd><dt className="max-w-[6.5rem] text-xs font-medium uppercase tracking-wider text-background/60">Snow response</dt></div>
            </dl>
          </div>
        </Container>
      </section>

      <section aria-label="Key credentials" className="border-b border-foreground/10 bg-secondary">
        <Container className="py-8 sm:py-10">
          <ul className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-3 lg:grid-cols-5">
            {["5.0★ Google rated", "15+ homeowner reviews", "Available 24/7", "Locally owned & operated", "Fully licensed & insured"].map((item) => <li key={item} className="flex items-center gap-2.5 text-sm font-medium text-foreground/80"><Check className="size-4 shrink-0 text-primary" />{item}</li>)}
          </ul>
        </Container>
      </section>
      ...
      <section className="overflow-hidden bg-background py-20 sm:py-28">
        <Container className="grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
          <Reveal>
            ...
            <h2 className="mt-3 text-3xl font-semibold tracking-tight text-foreground text-balance sm:text-4xl">A better yard starts with dependable care.</h2>
            <p className="mt-5 text-base leading-relaxed text-muted-foreground sm:text-lg">We keep the process simple: clear pricing, a real local crew, and the same care whether you need a weekly mow or a driveway cleared before sunrise.</p>
            <p className="mt-4 text-base leading-relaxed text-muted-foreground">We work respectfully around your property, clean up after every visit, and stay available through every Winnipeg season.</p>
            <ul className="mt-7 grid gap-2.5 sm:grid-cols-2">
              {["Fast quotes, usually within 24 hours", "Friendly, reliable crew", "Lower rates than the big guys", "Lawn and snow in one plan"].map((point) => <li key={point} className="flex items-center gap-2.5 text-sm font-medium text-foreground/80"><span className="grid size-5 place-items-center rounded-full bg-accent text-primary"><Check className="size-3" strokeWidth={3} /></span>{point}</li>)}
            </ul>
            <PrimaryCta href="/contact" className="mt-8">Request a free quote</PrimaryCta>
          </Reveal>
          <Reveal delay={0.1} className="relative">
            <Photo src="https://images.pexels.com/photos/27135591/pexels-photo-27135591.jpeg" alt="Landscaped residential yard in Winnipeg" ratio="4/3" className="rounded-lg" />
            <div className="absolute -bottom-5 left-5 right-5 rounded-md border border-foreground/5 bg-background p-4 shadow-soft sm:left-8 sm:right-auto sm:max-w-xs"><p className="text-sm font-semibold text-foreground">One crew. Every season.</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Summer mowing, fall cleanup, and winter snow clearing from people who know your property.</p></div>
          </Reveal>
        </Container>
      </section>
      ...
      <section className="bg-background py-20 sm:py-28"><Container><div className="max-w-2xl"><Eyebrow>Before & after</Eyebrow><h2 className="mt-3 text-3xl font-semibold tracking-tight text-foreground text-balance sm:text-4xl">A home that looks cared for again.</h2><p className="mt-4 text-base leading-relaxed text-muted-foreground sm:text-lg">Seasonal cleanup and regular lawn care make a visible difference — from the first pass to the finished Winnipeg yard.</p></div><div className="mt-12 grid gap-6 md:grid-cols-2"><Reveal className="group"><div className="overflow-hidden rounded-lg border border-foreground/5 bg-secondary"><div className="relative"><Photo src={CLEANUP_IMG} alt="Residential yard before seasonal cleanup" ratio="16/11" imgClassName="grayscale-[0.15] transition-transform duration-700 group-hover:scale-[1.03]" /><span className="absolute left-4 top-4 rounded-md bg-foreground/85 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-background">Before</span></div><div className="p-5"><h3 className="text-base font-semibold">Overgrown seasonal yard</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">Leaves, untidy beds, and edges that need a proper reset.</p></div></div></Reveal><Reveal delay={0.1} className="group"><div className="overflow-hidden rounded-lg border border-foreground/5 bg-secondary"><div className="relative"><Photo src="https://images.pexels.com/photos/27135591/pexels-photo-27135591.jpeg" alt="Residential yard after professional landscaping cleanup" ratio="16/11" imgClassName="transition-transform duration-700 group-hover:scale-[1.03]" /><span className="absolute left-4 top-4 rounded-md bg-primary px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-primary-foreground">After</span></div><div className="p-5"><h3 className="text-base font-semibold">Clean, ready-to-enjoy property</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">A tidier yard with the details handled and the season ready to begin.</p></div></div></Reveal></div></Container></section>
